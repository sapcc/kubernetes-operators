...
def seed_config(config, keystone):
...
def seed(args):
    ...
    try:
        logging.info("seeding keystone with '%s'" % config)

        plugin = cli.load_from_argparse_arguments(args)
        sess = session.Session(auth=plugin, user_agent='keystone-seeder')
        keystone = client.Client(session=sess, interface=args.interface)
        seed_config(config, keystone)
        return 0
    except Exception as e:
        logging.error("could not seed keystone: %s" % e)
