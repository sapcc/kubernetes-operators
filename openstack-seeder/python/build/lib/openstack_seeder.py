...
def seed_volume_type(volume_type, args, sess):
    """seed a cinder volume types"""
    logging.debug("seeding volume-type %s" % volume_type)

    # intialize cinder client
    try:
        api_version = api_versions.APIVersion("3.50")
        cinder = cinderclient.Client(session=sess, interface=args.interface, api_version=api_version)
    except Exception as e:
        logging.error("Fail to initialize cinder client: %s" % e)
        raise
    volumteType = cinder.volume_types.create(volume_type.name, volume_type.description, volume_type.is_public)
    ...
    if 'extra_specs' in volume_type:
        ...
        if not isinstance(extra_specs, dict):
            ...
            for k in extra_specs:
                volumteType.set_keys(k)
                #resource_classes.add(k.split(':', 2)[-1])
                logging.info("got volume_type: {}".format(k))
...
def seed_config(config, args, sess):
    ...
    if 'volume_types' in config:
        for volume_types in config['volume_types']:
            if volume_types:
                seed_volume_type(volume_types, args, sess)
